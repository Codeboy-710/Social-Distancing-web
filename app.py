...
with gr.Blocks(
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1, min_width=260, elem_id="sidebar"):
            ...
            gr.Image(value="cropped_circle_image.png", height=140, show_label=False)
           ...
           - 📹 Video upload + live webcam monitoring  
        ...
        with gr.Column(scale=3):
            ...
            with gr.Tabs():
                ...
                with gr.Tab("📹 Video Upload"):
                    ...
                    conf_v = gr.Slider(0.1, 1.0, value=0.45, label="Confidence")
                    dist_v = gr.Slider(20, 200, value=80, label="Distance Threshold")

                    video_btn = gr.Button("Analyze Video")

                    with gr.Row():
                        with gr.Column(scale=2):
                            video_output = gr.Image(label="Live Feed", elem_id="live-feed")
                        with gr.Column(scale=1):
                            analytics_box = gr.JSON(label="Analytics")
                ...
                with gr.Tab("📷 Webcam"):
                    ...
                    conf_w = gr.Slider(0.1, 1.0, value=0.45, label="Confidence")
                    dist_w = gr.Slider(20, 200, value=80, label="Distance")

                    with gr.Row():
                        with gr.Column(scale=2):
                            webcam_output = gr.Image(label="Live Feed")
                        with gr.Column(scale=1):
                            webcam_stats = gr.JSON(label="Analytics")
                ...
                with gr.Tab("🕘 History"):
                    ...
                    gr.Button("Refresh").click(get_history, outputs=history_box)
                    gr.Button("Delete Last").click(delete_last_history, outputs=history_box)
                    gr.Button("Clear").click(clear_history, outputs=history_box)
